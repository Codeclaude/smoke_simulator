...
:: Chemins pour ImGui (ajustez selon votre installation)
...
:: Verifier l'existence d'ImGui
if not exist "%IMGUI_DIR%" (
    echo Error: ImGui directory %IMGUI_DIR% does not exist.
    echo Please download ImGui and place it in %IMGUI_DIR%
    exit /b 1
)
...
call :CompileImGuiFile "%IMGUI_DIR%\imgui.cpp" imgui_core
call :CompileImGuiFile "%IMGUI_DIR%\imgui_draw.cpp" imgui_draw
call :CompileImGuiFile "%IMGUI_DIR%\imgui_tables.cpp" imgui_tables
call :CompileImGuiFile "%IMGUI_DIR%\imgui_widgets.cpp" imgui_widgets
call :CompileImGuiFile "%IMGUI_DIR%\imgui_demo.cpp" imgui_demo

:: Compiler les fichiers ImGui backends (SDL3)
echo Compiling ImGui backend files...
call :CompileImGuiFile "%IMGUI_BACKEND_DIR%\imgui_impl_sdl3.cpp" imgui_impl_sdl3
call :CompileImGuiFile "%IMGUI_BACKEND_DIR%\imgui_impl_sdlrenderer3.cpp" imgui_impl_sdlrenderer3

:: Compiler les .cpp du projet (recursivement)
...
for /r "%SRC_DIR%" %%f in (*.cpp) do (
    ...
    if "!o_date!"=="" (
        ...
        if "!cpp_date!" GTR "!o_date!" (
            ...
            :: Verifier si les fichiers .h inclus sont plus recents
    ...
    if "!compile!"=="yes" (
        ...
        "%COMPILER%" -c "!cpp_full!" -I "%SRC_DIR%" -I "%INCLUDE_DIR%" -I "%IMGUI_DIR%" -I "%IMGUI_BACKEND_DIR%" !CFLAGS! -o "!o!"
        if errorlevel 1 (
...
if "!OBJ_FILES!"=="" (
    echo Error: No object files generated for linking.
    exit /b 1
)

:: Lister tous les fichiers .o dans Build\Obj pour le linking
set "ALL_OBJ="
for %%f in ("%OBJ_DIR%\*.o") do (
    set "ALL_OBJ=!ALL_OBJ! %%f"
)

:: Lier tous les .o en executable
echo Linking...
echo Linking all object files from %OBJ_DIR%...
echo.
echo DEBUG - Object files to link:
dir /b "%OBJ_DIR%\*.o"
echo.
"%COMPILER%" !ALL_OBJ! -L "%LIB_DIR%" -lSDL3 -luser32 -lgdi32 -lwinmm -limm32 -lole32 -loleaut32 -lversion -luuid -ladvapi32 -lshell32 -o "%BIN_DIR%\%OUT%"
if errorlevel 1 (
...
:: Fonction pour compiler un fichier ImGui
:CompileImGuiFile FullPath BaseName
setlocal
set "cpp_full=%~1"
set "base=%~2"
set "o=%OBJ_DIR%\%base%.o"
...
if "%o_date%"=="" (
    set "compile=yes"
) else if "%cpp_date%" GTR "%o_date%" (
    set "compile=yes"
)

if "%compile%"=="yes" (
    echo Compiling %base%...
    "%COMPILER%" -c "%cpp_full%" -I "%IMGUI_DIR%" -I "%IMGUI_BACKEND_DIR%" -I "%INCLUDE_DIR%" !CFLAGS! -o "%o%"
    if errorlevel 1 (
        echo Error compiling %base%
        endlocal
        exit /b 1
    )
) else (
...
:: Verifier aussi les headers ImGui
